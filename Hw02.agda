...
module Hw02 where
  ...
  equality : (n m : Nat) → Either (Equals n m) (Void)
  ...
  plus-comm (1+ n) (1+ m) = {!sym (plus m (1+ n)) (plus n (1+ m))!}

  {- I'm fairly sure I'm supposed to use sym in this case. However, when I use sym,
     and refine it, another goal is automatically created. Believe I do not fully understand
     how to use sym within plus-comm as well as equality
  -}
  ...
  -}
  problem3 : (xs : List Nat) → Either (Σ[ n ∈ Nat ] In n xs × Even n) (AllOdd xs) 
  problem3 [] = Inr <>
  problem3 (x :: xs)
   with problem3 xs
  ... | Inl nIsEven = Inl {!!}
  ... | Inr nIsOdd = Inr {! nIsOdd!}
